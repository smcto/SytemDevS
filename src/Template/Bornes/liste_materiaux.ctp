...
$this->start('actionTitle');
    ...
    echo $this->Html->link('Export Pdf',['action'=>'listeMateriaux', 'pdf'],['escape'=>false,"class"=>"btn pull-right hidden-sm-down btn-rounded btn-success m-r-5" ]);
                            ...
                            <tbody>
                                <?php foreach ($bornes as $key => $borne): ?>
                                    <tr>
                                        ...
                                        <td>
                                            <?php if($borne->equipement_bornes) : ?>
                                                <?php  foreach ($borne->equipement_bornes as $equipement) : ?>
                                                    <?php if($equipement->numero_series) : ?>

                                                        <div class="col-md-8">
                                                            <?php if($equipement->equipement) : ?>
                                                                <?= $equipement->equipement->valeur ?> 
                                                                <?= @$equipement->numero_series->lot_produit->tarif_achat_ht? ' - ' . @$equipement->numero_series->lot_produit->tarif_achat_ht. ' €' : ''; ?>
                                                            <?php endif; ?>
                                                        </div>
                                                    <?php endif ?>
                                                <?php endforeach ?>
                                        ...
                                        <td><?= $this->Utilities->formatNumber(collection($borne->equipement_bornes)->sumOf('numero_series.lot_produit.tarif_achat_ht')). ' €'; ?> </td>
                                        <td>
                                            <div class="dropdown d-inline container-bornes-actions inner-table-menu">
                                                ...
                                                <div class="dropdown-menu customized-scrollbar" aria-labelledby="dropdownMenuButton">
                                                    <a class="dropdown-item" href="<?= $this->Url->build(['action' => 'edit', $borne->id]) ?>">Modifier fiche borne</a>
