...
<?php $this->start('bloc_btn') ?>
    <div class="float-right">
        <a href="javascript:void(0);" class="btn btn-sm btn-rounded btn-primary save">Enregistrer et continuer</a>
        <a href="javascript:void(0);" class="btn btn-sm btn-rounded btn-primary save_quit">Enregistrer et quitter</a>
        <a href="javascript:void(0);" class="btn btn-sm btn-rounded btn-primary save_print">Enregistrer et imprimer</a>
                    ...
                    <div class="table-responsive">
                        <table class="table">
                            ...
                            <tr>
                                ...
                                <th scope="col" class="<?= $bonsPrepa->bons_commande_id ? : 'hide' ?>">Restant à expédier </th>
                            ...
                            <?php foreach ($bonsPrepa->bons_commandes_produits as $key => $produit): $total += $produit->quantite ?>
                                ...
                                <tr class="tr-<?=$produit->statut ?>">
                                    ...
                                    <td>
                                        <?= $produit->reference ?>
                                        <?= $this->Form->hidden("bons_commandes_produits.$key.reference") ?>
                                        <?= $this->Form->hidden("bons_commandes_produits.$key.id") ?>
                                        <?= $this->Form->hidden("bons_commandes_produits.$key.i_position", ['input-name' => 'i_position', 'label' => false, 'class' => 'i-position']); ?>
                                    </td>
                                    <td>
                                        <?= $this->Form->control("bons_commandes_produits.$key.description_commercial", ['label' => false, 'class' => 'tinymce']) ?>
                                    </td>
                                    <td class="text-center">
                                        <?= $produit->quantite ?>
                                        <?= $this->Form->hidden("bons_commandes_produits.$key.quantite", ['class' => 'quantite']) ?>
                                    </td>
                                    <td class="text-center <?= $bonsPrepa->bons_commande_id ? : 'hide' ?>">
                                        <?= $produit->rest_a_livrer ?>
                                        <?= $this->Form->hidden("bons_commandes_produits.$key.rest_a_livrer", ['class' => 'rest_a_livrer']) ?>
                                    </td>
                                    <td>
                                        <?= $this->Form->control("bons_commandes_produits.$key.quantite_livree", ['input-name' => 'quantite_livree', 'label' => false, 'step' => 1, 'max' => $produit->rest_a_livrer, 'min' => 0, 'disabled' => $produit->statut == 'complet' ? true : false]) ?>
                                        <br>
                                    ...
                                    <td class="text-center hide">
                                        ...
                                        <?= $this->Form->hidden("bons_commandes_produits.$key.rest", ['class' => 'rest', 'value' => is_numeric($produit->rest) ? $produit->rest : $produit->rest_a_livrer]) ?>
                                    </td>
                                    <td>
                                        <?= $this->Form->control("bons_commandes_produits.$key.observation", ['label' => false, 'class' => 'tinymce']) ?>
                                    </td>
                                    <td>
                                        <div class="status-text <?=$produit->statut ?>"><?= @$statut_ligne[$produit->statut] ?></div>
                                        <?= $this->Form->hidden("bons_commandes_produits.$key.statut", ['label' => false, 'class' => 'status']) ?>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                            </tbody>
