...
<?php
$titrePage = "Modification d'un modèle de borne" ;
...
<div class="col-lg-12">
    <div class="card card-outline-info">
        ...
        <div class="card-body">
            <?= $this->Form->create($modelBorne, ['type' => 'file']) ?>
                <div class="form-body">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input type="date" name="date_sortie" class="form-control" placeholder="dd/mm/yyyy" required="" value="<?= $modelBorne->date_sortie->format('Y-m-d') ?>">
                            </div>
                        </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="hidden" value="<?= $modelBorne->id ?>" id="modelborne_id">
                        </div>
                    </div>
                    <!--/row-->
                    ...
                    <div class="">
                        ...
                        foreach($modelBorne->dimensions as $key => $dimension){ 
                            $labelPartie = "Partie";
                            ...
                            */
                            ...
                            <!-- <div class="row kl_row_<?= $key ?>">
                                <div class="col-md-4 ">
                                    <?php // echo $this->Form->hidden('dimensions.'.$dimension->id.'.id', ['label'=>$labelPartie, 'type'=>'select', 'options'=>$parties,'value'=>$dimension->id]); ?>
                                   <?php  //echo $this->Form->control('dimensions.'.$dimension->id.'.partie_id', ['label'=>$labelPartie, 'type'=>'select', 'options'=>$parties,'value'=>$dimension->partie_id]); ?>
                                </div>
                                <div class="col-md-4 ">
                                    <?php //echo $this->Form->control('dimensions.'.$dimension->id.'.dimension',['label' => $labelDimension,'value'=>$dimension->dimension]); ?>
                                </div>
                                <div class="col-md-4 ">
                                    <?php //echo $this->Form->control('dimensions.'.$dimension->id.'.poids',['label' => $labelPoids,'value'=>$dimension->poids]); ?>
                                </div>
                            </div>
                            -->
                        <?php //  } ?>
                    ...
                    <h3 class="box-title m-t-40">Equipements </h3>
                    <hr>
                    ...
                    <hr>
                    <?php if (!empty($modelBorne->documents)){ ?>
                    <?php foreach ($modelBorne->documents as $key => $document){ ?>
                    ...
                    <div class="row p-t-20 gestionDocuments" id="gestionDocuments-<?= $key ?>">
                        <?php echo $this->Form->control('documents.'.$key.'.id',["id"=>"documents-id-".$key,"type"=>"hidden"]) ?>
                        <div class="col-md-3 bloc_dropify">
                            <?php echo $this->Form->control('documents.'.$key.'.file',["id"=>"documents-file-".$key, "label"=>"Fichier : ","class"=>"dropify", "type"=>"file", "data-height"=>"100", 'data-default-file'=>$document->url]) ?>
                            <?php  if(!empty($document->url)) {
                            echo $this->Html->link('Visualiser', $document->url,['escape' => false,"class"=>"", 'target'=>'_blank']);
                            }
                            ?>
                        </div>
                        <div class="col-md-3 bloc_titre">
                            <?php echo $this->Form->control('documents.'.$key.'.titre',["id"=>"documents-titre-".$key, 'label' => 'Titre : ', 'type' => 'text']); ?>
                        </div>
                        <div class="col-md-4">
                            <?php echo $this->Form->control('documents.'.$key.'.description',["id"=>"documents-description-".$key, 'label' => 'Description : ', "class"=>"form-control", 'type' => 'textarea']); ?>
                        </div>
