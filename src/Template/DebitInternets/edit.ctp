...
<!--<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        ...
        <li><?= $this->Form->postLink(
                __('Delete'),
                ['action' => 'delete', $debitInternet->id],
                ['confirm' => __('Are you sure you want to delete # {0}?', $debitInternet->id)]
            )
        ?></li>
...
<div class="debitInternets form large-9 medium-8 columns content">
    <?= $this->Form->create($debitInternet) ?>
    <fieldset>
        <legend><?= __('Edit Debit Internet') ?></legend>
        <?php
