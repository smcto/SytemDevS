...
<?php $this->start('actionTitle') ?>
    ...
    <a href="<?= $this->Url->build(array_filter($customFinderOptions)) ?>" class="btn pull-right hidden-sm-down btn-rounded btn-primary mr-2">Export en csv</a>
...
<div class="modal fade" id="form_client_2" tabindex="-1" role="dialog"  aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <?= $this->Form->create($devisEntity, ['url' => ['action' => 'lierClient2'] ,'id' => 'client_2_form']); ?>
                <div class="modal-header">
                    ...
                    <div class="">
                        <div class="row row-client">
                            ...
                            <div class="col-md-9">
                                ...
                                <input type="hidden" name="devi_id" id="devi_id_2">
                            </div>
                        </div>
                    </div>
                </div>
...
<div class="modal fade" id="edit_commercial" tabindex="-1" role="dialog"  aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <?= $this->Form->create($devisEntity, ['url' => ['action' => 'editCommercial'] ,'id' => 'edit_commercial_form']); ?>
                <div class="modal-header">
...
<div class="modal fade" id="bons_commande" tabindex="-1" role="dialog"  aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <?= $this->Form->create(null, ['url' => ['action' => 'editClient'] ,'id' => 'edit_client_form']); ?>
                <div class="modal-header">
                    <h5 class="modal-title" id="title">Informations sur la commande </h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"> <span aria-hidden="true">&times;</span> </button>
                </div>
                <div class="modal-body">
                    <div class="row">
                        <label class="col-md-4">Date de livraison souhaitée : </label>
                        <div class="col-md-8">
                            <?= $this->Form->control('type_date', ['type' => 'select', 'options' => $type_date, 'empty' => 'Sélectionner', 'label' => false,'style' => 'width:100%',"data-placeholder"=>"Sélectionner",'required', 'class' => 'selectpicker']) ?>
                            <input type="hidden" name="devi_id" id="bl_devi_id">
                        </div>
                    </div>

                    <div class="row hide date-bc">
                        <label class="col-md-4">Date : </label>
                        <div class="col-md-8">
                            <?= $this->Form->text('date', ['type' => 'date', 'class' => 'form-control','label' => false]); ?>
                        </div>
                    </div>
                    <br>
                    <div class="row">
                        ...
                        <div class="col-md-8">
                            <?= $this->Form->control('commentaire', ['type' => 'textarea', 'class' => 'tinymce', 'label' => false]) ?>
                        </div>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-rounded btn-dark" data-dismiss="modal" aria-label="Close"> <span aria-hidden="true">Annuler</span> </button>
                    <?= $this->Form->button(__('Save'), ['class' => 'btn btn-rounded btn-success btn-submit-client','escape' => false]) ?>
                </div>
            <?= $this->form->end() ?>
        </div>
    </div>
                        ...
                        <table class="table table-striped devis-table liste-items" id="div_table_bornes">
                            <thead>
                                <tr>
                                    ...
                                    <th class="th-numero"><?= $this->Paginator->sort('indent', 'N°') ?></th>
                                    <th class="th-client"><?= $this->Paginator->sort('Clients.nom', 'Client/Prospect') ?></th>
                                    ...
                                    <th class="th-document"><?= $this->Paginator->sort('DevisTypeDocs.nom', 'Document') ?></th>
                                    <th><?= $this->Paginator->sort('Commercial.nom', 'Contact') ?></th>
                                    <th class="th-date"><?= $this->Paginator->sort('date_crea', 'Date devis') ?></th>
                                    ...
                                    <!-- <th>Règlement</th> -->
                                    <th class="th-expire"><?= $this->Paginator->sort('date_sign_before', 'Expire') ?></th>
                            ...
                            <tbody>
                                <?php foreach ($listeDevis as $key => $devis): ?>
                                    ...
                                    <tr sellsy-doc-id="<?= $devis->sellsy_doc_id ?>" sellsy-client-id="<?= $devis->sellsy_client_id ?>">
                                        ...
                                        <td class="td-etat"><div class="table-status-wrap"><i class="fa fa-circle <?= $devis->status ?> font-12" data-toggle="tooltip" data-placement="top" title="<?= @$devis_status[$devis->status] ?>" data-original-title="Brouillon"></i> <?= @$devis_status[$devis->status] ?></div></td>
                                        <td>
                                            <?php if (!$devis->is_in_sellsy): ?>
                                                <div class="dropdown d-inline container-ventes-actions inner-table-menu">
                                                    ...
                                                    <div class="dropdown-menu customized-scrollbar" aria-labelledby="dropdownMenuButton">
                                                        ...
                                                        <a href="javascript:void(0);" class="dropdown-item <?= $devis->is_consommable ? : 'hide' ?>" data-toggle="modal" data-target="#bons_commande" data-href="<?= $this->Url->build(['controller' => 'BonsCommandes', 'action' => 'CreateBcByDevis', $devis->id]) ?>" data-indent='<?= $devis->indent ?>' data-devis="<?= $devis->id ?>">Valider et créer BC</a>
                                                        ...
                                                        <!--a href="javascript:void(0);" class="dropdown-item" data-toggle="modal" data-target="#form_client_2" data-title="Lier à un 2ème client (<?= $devis->indent ?>)" data-devi="<?= $devis->id ?>" >Lier à un 2ème client </a -->
                                                        <a href="javascript:void(0);" class="dropdown-item" data-toggle="modal" data-target="#edit_commercial" data-title="Changer de commercial (<?= $devis->indent ?>)" data-devi="<?= $devis->id ?>" >Changer le commercial</a>
