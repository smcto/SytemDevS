<?= $this->Html->script('devis/model_list.js?time='.time(), ['block' => true]); ?>
        ...
        <div class="card">
            <div class="card-body">
                <?= $this->Form->create(false, ['type' => 'GET', 'class' => 'mt-4']); ?>
                    ...
                    <div class="row">
                        <div class="col-md-2">
                            <?= $this->Form->control('keyword_model', ['label' => false, 'default' => $keyword, 'placeholder' => 'Rechercher','id'=>'id_keyword']); ?>
                        </div>
                        
                        <div class="col-md-2">
                            <?= $this->Form->control('model_type', ['options' => $type_bornes, 'label' => false, 'default' => $model_type, 'empty' => 'Type du modèle ']); ?>
                        </div>
                    ...
                    <tbody>
                        <?php foreach ($listeDevis as $key => $devis): ?>
                            <tr>
                                ...
                                <td><?= $devis->modele_devis_category?$devis->modele_devis_category->name:"" ?> <?= $devis->modele_devis_sous_category?" / " . $devis->modele_devis_sous_category->name : ""?></td>
                                <td class="text-right"><?= $devis->get('TotalHtWithCurrency') ?></td>
                                <td class="text-right"><?= $devis->get('TotalTtcWithCurrency') ?></td>
                                <td><i class="fa fa-circle <?= $devis->status ?> font-12" data-toggle="tooltip" data-placement="top" title="<?= @$devis_status[$devis->status] ?>" data-original-title="Brouillon"></i> </td>
                                <td>
                                    <div class="dropdown d-inline container-ventes-actions">
                                        ...
                                        <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                            ...
                                            <a class="dropdown-item" href="<?= $this->Url->build(['action' => 'duplicatDevis', $devis->id, 'is_model' => 1]) ?>">Dupliquer le document</a>
                                            <?= $this->Form->postLink('Supprimer', ['action' => 'delete', $devis->id], ['class' => 'dropdown-item', 'escape' => false, 'confirm' => 'Êtes-vous sûr de vouloir supprimer ?'] ); ?>
                                        </div>
                                    </div>
                                </td>
                            </tr>
