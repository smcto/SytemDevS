$(document).ready(function() {
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data.status == 1) {
                    ...
                    $('#devis_client_adress').modal('hide');
                }
            },
            error: function(data) {
    ...
    $('.no-spin').unbind('blur').on('blur', '#date_validite', function(event) {
        echeanceUpdate();
    });
...
// ne marche pas correctement dans addTr
...
function echeanceUpdate() {
    ...
    var montant_total = $('.total_general_ttc').text();
    var validity = $('#date_validite').val();
    var montant = 0;
    if (montant_total !== '') {
        montant = parseFloat(montant_total / number).toFixed(2);
    }
    
    var now = new Date(Date.now());
    if(validity !== '') {
        now = new Date(validity);
    }
    
    $('tbody.echeance').find('tr').each(function(index, el) {
        var percent = $(this).find('[input-name="percent_value"]').val();
        if(percent !== '') {
            montant = parseFloat(montant_total * percent / 100).toFixed(2);
        }
