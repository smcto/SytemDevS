/**
 * setDropdown dans devis-factures/autre.js
 */
$(window).bind("load", function() {
    ...
    function majRecap() {
        ...
        tbody.find('[input-name="prix_reference_ht"]').each(function(index, el) {
            ...
            if( ! $(this).parents('tr').hasClass('ligne-option')) {
                ...
                divUnity = currentTr.find('#devis-factures-produits-catalog-unite-id');
                unity = divUnity.children("option:selected").val();

                montantHT = currentTr.find('td.montant_ht .montant_ht_value').text();
    ...
    function setSublineBehavior() {
        ...
        // Ajout abonnement
        createTrLineOnClick({'classTarget': '.ajout-dessus a.abonnement', 'position': 'commentBeforeCurrentLine', 'classType': 'clone-abonnement'})
        createTrLineOnClick({'classTarget': '.ajout-dessous a.abonnement', 'position': 'commentAfterCurrentLine', 'classType': 'clone-abonnement'})
    ...
    function createTrLineObjectOnClick(params) {
        tbody.on('click', params.classTarget, function(event) {
            ...
            var btnModalSubmit = $('#devis_factures_catalog').find('button#submit-catalog');
            $('#position').val(params.position);

            $('#devis_factures_catalog').modal('show');
            $('#devis_factures_catalog').on('shown.bs.modal', function () {
    ...
    function createTrLineOptionOnClick() {
    ...
    function removeTrLineOptionOnClick() {
    ...
    function createTrLineAbonnementOnClick() {
    ...
    function removeTrLineAbonnementOnClick() {
    ...
    $('#submit-catalog').on('click', function(event) {
        ...
        var currentTr = $('.table_bloc_devis_factures .default-data tr').get(currentTrIndex);
    ...
    function setProductAfterSubmitingModal(key, tr = null, targetBtnModalSubmit = null) {
        ...
        if (content === undefined) {
            $('#devis_factures_catalog').modal('hide');
    ...
    $('.annuler_modele').on('click', function(event) {
        ...
        $('#modele-devis-factures-category-id').prop('required', false);
        $('#modele-devis-factures-sous-category-id').prop('required', false);
        $('#model_devis_factures').modal('hide');
    });
    
    $('.save_model').on('click', function(event) {
        $('#model-name').prop('required', true);
        $('#modele-devis-factures-category-id').prop('required', true);
        $('#modele-devis-factures-sous-category-id').prop('required', true);
    });

    // deinfition ordre avant save
    $('.save_devis_factures').on('click', function(event) {
        event.preventDefault();
        submit();
    });

    $('.save_continuer_devis_factures').on('click', function(event) {
        event.preventDefault();
        $('#is_continue').val(1);
        submit();
    });
    ...
    $('.model_submit').on('click', function() {
        $('#is_model').val(1);
        if($('#model-name').val() != '' && $('#modele-devis-factures-category-id').val() != '') {
            event.preventDefault();
            submit();
        }
    });
    ...
    function submit() {
        ...
        if($('#objet').val() == '') {
            alert('Objet de la facture doit être non vide');
        } else {
            $('.table_bloc_devis_factures tfoot').remove();

            $('.table_bloc_devis_factures tbody.default-data tr').each(function(index, el) {
                trIndex = index;
                currentTr = $(this);
                trInputName = currentTr.find('[input-name]');

                trInputName.each(function(index, el) {
                    $(this).attr('name', 'devis_factures_produits[' + trIndex + '][' + $(this).attr('input-name') + ']');
                });
    ...
    $("#remise-line").on('click', function(){
        if($(this).is(':checked')){
            $('.remise-global').prop('disabled', true);
            $('#remise-global-value').val(0);
            $('.total-remise-global').addClass('hide');
            $('input[input-name="remise_value"], select[input-name="remise_unity"]').prop('disabled', false);
        }else {
            $('.remise-global').prop('disabled', false);
            $('.total-remise-global').removeClass('hide');
    ...
    $("#remise-hide-line").on('click', function(){
        if($(this).is(':checked')){
    ...
    $('#modele-devis-factures-categories-id').on('change', function() {
        catId = $(this).val();
        sCatId = $('#sCatId').val();
        ...
        $('#modele-devis-factures-sous-categories-id').html(option);
    });
    $('#modele-devis-factures-categories-id').trigger('change');
    ...
    $('#ref-commercial-id').on('change', function() {
        ...
        if (idCommercial > 0) {
            $.ajax({
                url: srcUrl + 'fr/ajax-devis-factures/changeAddInfo',
                type: "POST",
                dataType: 'json',
                data: {
                ...
                success: function(data) {
                    ...
                    if (data.status == 1) {
                        ...
                        $('#telephone_fixe').html(data.user.telephone_fixe);
...
//        if($(this).is(':checked')){
//            $('.tva').addClass('hide');
//            $('th.tva .visibility-mark').addClass('isInnactive');
//            $('.dynamic-colspan').each(function() {
//                cols = $(this).prop('colspan');
//                $(this).prop('colspan',cols - 1);
//            });
//        }else {
//            $('.tva').removeClass('hide');
//            $('th.tva .visibility-mark').removeClass('isInnactive');
//            $('.dynamic-colspan').each(function() {
//                cols = $(this).prop('colspan');
//                $(this).prop('colspan',cols + 1);
//            });
//        }
//        
//        name = $('th.tva .visibility-mark').attr('data');
//        isInnactive = $('th.tva .visibility-mark').hasClass('isInnactive');
//        if (isInnactive) {
//            $('th.tva .visibility-mark').find('.fa').removeClass('fa-eye').addClass('fa-eye-slash');
//        } else {
//            $('th.tva .visibility-mark').find('.fa').removeClass('fa-eye-slash').addClass('fa-eye');
//        }
//        dataVisibilityParams[name] = $('th.tva .visibility-mark').hasClass('isInnactive');
//        $('[name="col_visibility_params"]').val(JSON.stringify(dataVisibilityParams));
//    });
