...
$(document).ready(function() {
    ...
    $('.tinymce').tinymce({
     ...
     $("body" ).on( "focusout", "#montant", function(e) {
        var total_ttc = $('#total_ttc').val();
        var montant_reglement = $(this).val();
        if(total_ttc <= montant_reglement) {
            $('#montant_lie').val(total_ttc);
        } else {
            $('#montant_lie').val(montant_reglement);
        }
    });
    ...
    $('#change_status').on('change', function() {
        ...
        var value = $(this).val();
        $.ajax({
            url: srcUrl + 'fr/ajax-devis-factures/edit-etat/' + devis_id,
            type: "POST",
            dataType: 'json',
            data: {
                status: value
