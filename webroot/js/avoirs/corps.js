...
$(window).bind("load", function() {
    ...
    function addNewLineLine(position = 'bottom', tr = null, data = null, class_type = 'clone') {
        ...
        clonedTr = $('tfoot tr.' + class_type).last().clone();
        placeholder = '';
    ...
    function createTrLineObjectOnClick(params) {
        tbody.on('click', params.classTarget, function(event) {
            curentTrForObject = $(this).parents('tr');
            $('#position').val(params.position);
            $('#devis-factures_catalog').modal('show');
        });
    }
    ...
    function setProductAfterSubmitingModal(key) {
        ...
        $.get(srcUrl + 'fr/catalog-produits/get-produit-by-id/' + id, function(data, xhr) {
            $('tbody').removeAttr('style');
            $('.detail-form').removeClass('d-none');
            addNewLineLine('bottom', null, data).promise().then(function (e) {
                setProductAfterSubmitingModal(key+1);
            });
        });
    }
    // -------- end ----------
    ...
    $('.save_avoir').on('click', function(event) {
        event.preventDefault();
        submit();
    });

    $('.save_continuer_avoir').on('click', function(event) {
        event.preventDefault();
        $('#is_continue').val(1);
        submit();
    });
    ...
    function submit() {
        ...
        if($('#objet').val() == '') {
            ...
            alert('L\'objet doit être non vide');
        } else if($('#facture_id').val() == '') {
            
            alert('Veuillez lier à une facture');
        } else {
            ...
            $('.table_bloc_devis_factures tbody.default-data tr').each(function(index, el) {
                ...
                trInputName.each(function(index, el) {
                    $(this).attr('name', 'avoirs_produits[' + trIndex + '][' + $(this).attr('input-name') + ']');
                });
    ...
    $("#remise-line").on('click', function(){
        ...
        $('input[input-name="remise_value"]').trigger('blur');
        $('#remise-hide-line').trigger('click');
        showTrRemiseGlobal();
        majRecap();
    });
