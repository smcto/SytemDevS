$(document).ready(function() {
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.status == 1) {
                    ...
                    $('#edit_client_adress').val(data.client.adresse);
                    $('#edit_client_adress_2').val(data.client.adresse_2);
                    $('#edit_client_cp').val(data.client.cp);
                    $('#edit_client_ville').val(data.client.ville);
                    $('#edit_client_country').val(data.client.country);
    ...
    $('form#facture_form').on('submit', function (e) {
        e.preventDefault();
        var facture = $('#devis-facture-id').val();
        if (facture != '') {
            $('#facture_id').val(facture);
            $('.facture_indent').text('Lié à la facture : ' + factures[facture]);
            $('.edit_facture_text').text('Modifier');
        } else {
            $('.facture_indent').text('');
            $('.edit_facture_text').text('Lier à une facture');
        }
        $('#edit_facture').modal('hide');
    });
